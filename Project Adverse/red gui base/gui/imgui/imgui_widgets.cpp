...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImU32 col = ImColor(20, 20, 20);
...
bool ImGui::cCheckbox(const char* label, bool* v, ImColor C) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // modified by qo0
    PushStyleColor(ImGuiCol_Border, GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonActive : ImGuiCol_Border));
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
    PopStyleColor();
    ImU32 check_col = ImColor(79, 192, 249);

    if (window->DC.ItemFlags & ImGuiItemFlags_MixedValue)
    {
        // Undocumented tristate/mixed/indeterminate checkbox (#2644)
        ImVec2 pad(ImMax(1.0f, IM_FLOOR(square_sz / 3.6f)), ImMax(0.0f, IM_FLOOR(square_sz / 3.6f)));
        window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, C, style.FrameRounding);
    }
    else if (*v)
    {
        // checkboxs
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(0, 0), check_bb.Max - ImVec2(0, 0), GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
        // gradient
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        //window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.40f)), GetColorU32(ImVec4(0.8f, 0.8f, 0.8f, 0.2f)));
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, C, 2.0f, 15);

        //RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 1.0f);
    }

    ImColor TextCol = ImColor(255, 255, 255, 255);
    ...
    if (label_size.x > 0.0f)
        window->DrawList->AddText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y), TextCol, label);
...
{
    static std::map<ImGuiID, float> text_selection;
    ...
    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos - ImVec2(3, 0);
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id))
        return false;
    ...
    auto text_selected = text_selection.find(id);
    if (text_selected == text_selection.end())
    {
        text_selection.insert({ id, {0.f} });
        text_selected = text_selection.find(id);
    }

    text_selected->second = ImClamp(text_selected->second + 0.5f * ImGui::GetIO().DeltaTime * (*v ? 7.f : -7.2f), -0.2f, 1.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    ImU32 check_col = ImColor(0.737f, 0.552f, 0.760f, 1.0f * text_selected->second * 6.f); 

    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1, 0.8f), check_bb.Max - ImVec2(3, 4), ImColor(25, 25, 25, 255), 2.f);
    window->DrawList->AddRect(check_bb.Min + ImVec2(1, 0.8f), check_bb.Max - ImVec2(2, 2), ImColor(10, 10, 10), 2.f);
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1, 0.8f), check_bb.Max - ImVec2(3, 4), check_col, 2.f);

    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, 1.0f));
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 1, check_bb.Min.y + ((check_bb.Max.y - check_bb.Min.y) / 2) - label_size.y / 2 - 1), label);
    ImGui::PopStyleColor(1);

    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 0.4f * text_selected->second * 3));
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 1, check_bb.Min.y + ((check_bb.Max.y - check_bb.Min.y) / 2) - label_size.y / 2 - 1), label);
    ImGui::PopStyleColor(1);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        //RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 3), ImU32(ImColor(43, 198, 172, 255)) , ImGuiDir_Down, 0.5f);
    }
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    const float grab_padding = 0.001f;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        //slider grab
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, ImColor(188, 141, 194) ,(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 0);
        // gradient
        //window->DrawList->AddRectFilledMultiColor(grab_bb.Min, grab_bb.Max, GetColorU32(ImVec4(1.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(1.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(1.0f, 0.0f, 0.0f, 0.38f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f)));
        //window->DrawList->AddRectFilledMultiColor(grab_bb.Min, grab_bb.Max, GetColorU32(ImVec4(1.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(1.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(1.0f, 0.0f, 0.0f, 0.38f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f)));
    }
...
bool ImGui::cSliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, ImColor C , const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, 4.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        //slider grab
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, C, 2.f);
        // gradient
...
bool ImGui::cSliderFloat(const char* label, float* v, float v_min, float v_max, ImColor C, const char* format, ImGuiSliderFlags flags)
{
    return cSliderScalar(label, ImGuiDataType_Float, v, &v_min, C, &v_max, format, flags);
}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderActive : ImGuiCol_Header);
        RenderFrame(bb.Min, bb.Max, col, true, 8.0f);
